...
Item {
    ...
    Kicker.RunnerModel {
        ...
        runners: {
            const runners = ["services", "krunner_systemsettings"];

            if (kicker.isDash) {
                runners.push("desktopsessions", "PowerDevil", "calculator", "unitconverter");
            }

            if (plasmoid.configuration.useExtraRunners) {
                runners.push(...plasmoid.configuration.extraRunners);
            }
