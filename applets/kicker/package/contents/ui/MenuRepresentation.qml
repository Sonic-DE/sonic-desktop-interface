...
FocusScope {
    ...
    KSvg.FrameSvgItem {
        ...
        SideBarSection {
            ...
            KeyNavigation.right: runnerColumns.visible ? searchField : rootList
    ...
    Item {
        ...
        Row {
            ...
            readonly property Item firstVisibleColumn: visibleColumnAtIndex(0)
            readonly property Item secondVisibleColumn: visibleColumnAtIndex(1)

            function visibleColumnAtIndex(index) {
                if (!visible) {
                    return null;
                }
                let currentIndex = 0;
                for (let child of runnerColumns.children) {
                    if (child instanceof RunnerResultsList && child.visible) {
            ...
            onVisibleChanged: if (visible) {
                rootList.currentIndex = -1; // Focus on searchField
            }
                ...
                delegate: RunnerResultsList {
                    ...
                    Keys.onLeftPressed: event => {
                        ...
                        if (leftIndex >= 0) {
                            ...
                            rootList.Keys.onLeftPressed(event); // Focus on the first sidebar item
                        }
                    }
        ...
        ItemListView {
            ...
            Keys.onLeftPressed: event => {
                // Focus on the first sidebar item
                if (favoriteApps.repeater.itemAt(0) !== null) {
                    currentIndex = -1;
                    favoriteApps.repeater.itemAt(0).forceActiveFocus(Qt.TabFocusReason);
                } else if (favoriteSystemActions.repeater.itemAt(0) !== null) {
                    currentIndex = -1;
                    favoriteSystemActions.repeater.itemAt(0).forceActiveFocus(Qt.TabFocusReason);
                } else {
    ...
    PlasmaExtras.SearchField {
        ...
        KeyNavigation.up: runnerColumns.visible ? runnerColumns.firstVisibleColumn : rootList
        KeyNavigation.down: runnerColumns.visible ? runnerColumns.firstVisibleColumn : rootList
        KeyNavigation.left: favoriteApps.repeater.count > 0 ? favoriteApps : favoriteSystemActions
        KeyNavigation.right: runnerColumns.visible ? runnerColumns.secondVisibleColumn : null
        ...
        Keys.onUpPressed: event => {
            if (runnerColumns.firstVisibleColumn) {
                runnerColumns.firstVisibleColumn.currentIndex = runnerColumns.firstVisibleColumn.count - 1;
            } else {
        ...
        Keys.onDownPressed: event => {
            if (runnerColumns.firstVisibleColumn) {
                runnerColumns.firstVisibleColumn.currentIndex = 0;
            } else {
                // Create child dialogs only after a key is pressed
                rootList.showChildDialogs = false;
        ...
        Keys.onRightPressed: event => {
            if (runnerColumns.secondVisibleColumn) {
                runnerColumns.secondVisibleColumn.currentIndex = 0;
            }
        ...
        Keys.onReturnPressed: event => {
            if (runnerColumns.firstVisibleColumn) {
                (runnerColumns.firstVisibleColumn.listView.currentItem || runnerColumns.firstVisibleColumn.listView.itemAtIndex(0)).Keys.onReturnPressed(event);
            } else {
                event.accepted = false;
            }
        }
