...
GroupSortProxy::GroupSortProxy(QAbstractItemModel *sourceModel) : QSortFilterProxyModel(nullptr)
{
    ...
    setSortRole(ResultModel::TitleRole);
...
bool GroupSortProxy::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
    ...
    if (lResource.startsWith(QStringLiteral("applications:"))
        && !rResource.startsWith(QStringLiteral("applications:"))) {
        return true;
    } else if (!lResource.startsWith(QStringLiteral("applications:"))
        && rResource.startsWith(QStringLiteral("applications:"))) {
        return false;
    }
