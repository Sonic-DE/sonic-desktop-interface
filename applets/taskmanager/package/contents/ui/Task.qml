...
MouseArea {
    ...
    Accessible.description: {
        ...
        if (taskProgressOverlayLoader.active) {
            smartLauncherDescription += i18nc("@info:tooltip", "The current progress is %1 percent.", task.smartLauncherItem.progress);
        }
        if (taskBadgeOverlayLoader.active) {
            smartLauncherDescription += i18nc("@info:tooltip", "There are %1 new messages.", task.smartLauncherItem.count);
        }
