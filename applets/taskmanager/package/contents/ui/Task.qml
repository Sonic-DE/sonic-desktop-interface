        ...
        DragHandler {
            ...
            onActiveChanged: if (active) {
                icon.grabToImage((result) => {
                    ...
                    dragHelperItem.Drag.imageSource = result.url;
                    dragHelperItem.Drag.mimeData = dragHelper.generateMimeData(model.MimeType, model.MimeData, model.LauncherUrlWithoutIcon);
                    dragHelperItem.Drag.active = dragHandler.active;
                });
            } else {
                dragHelperItem.Drag.active = false;
                dragHelperItem.Drag.imageSource = "";
            }
        }
    }
