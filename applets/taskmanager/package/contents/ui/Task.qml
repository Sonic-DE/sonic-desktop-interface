...
MouseArea {
    ...
    Keys.onLeftPressed: if (!inPopup && event.modifiers & (Qt.ControlModifier | Qt.ShiftModifier)) {
        tasksModel.move(task.itemIndex, task.itemIndex - 1);
    }
    Keys.onRightPressed: if (!inPopup && event.modifiers & (Qt.ControlModifier | Qt.ShiftModifier)) {
        tasksModel.move(task.itemIndex, task.itemIndex + 1);
    }
