...
PlasmaCore.ToolTipArea {
    ...
    height: Math.max(Kirigami.Units.iconSizes.sizeForLabels, Kirigami.Units.iconSizes.medium) + taskList.verticalMargins
    ...
    onIsWindowChanged: {
        if (model.IsWindow) {
            ...
            if (!inPopup) {
                Qt.callLater(taskList.positionViewAtEnd);
            }
        }
    }
    ...
    KSvg.FrameSvgItem {
        ...
        anchors {
            ...
            topMargin: (!tasks.vertical && taskList.count > 1) ? LayoutManager.iconMargin : 0
            bottomMargin: (!tasks.vertical && taskList.count > 1) ? LayoutManager.iconMargin : 0
            leftMargin: ((inPopup || tasks.vertical) && taskList.count > 1) ? LayoutManager.iconMargin : 0
            rightMargin: ((inPopup || tasks.vertical) && taskList.count > 1) ? LayoutManager.iconMargin : 0
        }
    ...
    Loader {
        ...
        function adjustMargin(vert, size, margin) {
            ...
            var margins = vert ? taskList.horizontalMargins : taskList.verticalMargins;
