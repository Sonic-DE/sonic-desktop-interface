...
function wheelActivateNextPrevTask(anchor, wheelDelta, eventDelta, wheelSkipMinimized, windowViewAvailable, tasks) {
    ...
    while (increment != 0) {
        activateNextPrevTask(anchor, increment < 0, wheelSkipMinimized, windowViewAvailable, tasks)
...
function activateNextPrevTask(anchor, next, wheelSkipMinimized, windowViewAvailable, tasks) {
...
function taskPrefixHovered(prefix, location) {
    var effectivePrefix = taskPrefix(prefix, location);

    if ("" !== prefix)
        effectivePrefix = [
            ...taskPrefix(prefix + "-hover", location),
            ...taskPrefix("hover", location),
            ...effectivePrefix
        ];
