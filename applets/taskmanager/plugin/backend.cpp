...
static const QString appViewName = QStringLiteral("org.kde.KWin.Effect.AppView1");
static const QString appViewPath = QStringLiteral("/org/kde/KWin/Effect/AppView1");
...
{
    m_appViewAvailable = QDBusConnection::sessionBus().interface()->isServiceRegistered(appViewName);
    ...
    connect(watcher, &QDBusServiceWatcher::serviceRegistered, this, [this] {
        m_appViewAvailable = true;
        Q_EMIT appViewAvailableChanged();
    });
    connect(watcher, &QDBusServiceWatcher::serviceUnregistered, this, [this] {
        m_appViewAvailable = false;
        Q_EMIT appViewAvailableChanged();
    });
}
...
bool Backend::appViewAvailable() const
{
    return m_appViewAvailable;
}

void Backend::activateAppView(const QVariant &_winIds)
{
