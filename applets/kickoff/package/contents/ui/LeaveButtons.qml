...
RowLayout {
    id: root
    // Can't use implicitWidth - spacer.implicitWidth because buttons will still be invisible when switching from "Power and Session" to other options
    readonly property real buttonImplicitWidth: {
        let accumulatedWidth = 0;
        for (let i = 0; i < buttonRepeater.count; ++i) {
            const item = buttonRepeater.itemAt(i);
            accumulatedWidth += item.visible ? item.implicitWidth : 0;
        }
