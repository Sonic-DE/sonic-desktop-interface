...
KickoffGridView {
    ...
    view.cellWidth: {
        const sizeFromFont = Math.round(KickoffSingleton.listDelegateContentHeight * 2.5);
        const rowCount = Math.floor(view.availableWidth / sizeFromFont);
        return Math.floor(view.availableWidth / rowCount);
    }
