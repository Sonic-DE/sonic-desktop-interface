...
T.ItemDelegate {
    ...
    MouseArea {
        ...
        drag {
            ...
            minimumX: 0
            maximumX: root.view ? Math.min(root.parent.width - root.width, root.view.availableWidth - root.width) : 0
            minimumY: 0
            maximumY: root.view ? Math.min(root.parent.height - root.height, root.view.availableHeight - root.height) : 0
        }
        Binding {
        ...
        onPressAndHold: if (root.lastDeviceType === PointerDevice.TouchScreen) {
            root.forceActiveFocus(Qt.MouseFocusReason)
            root.pressAndHold()
            root.openActionMenu(mouseX, mouseY)
        }
