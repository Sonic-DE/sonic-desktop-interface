...
AbstractButton {
    ...
    background: Item {
        ...
        property int state: {
            if (controlRoot.down) {
                return MenuButton.State.Down
            } else if (controlRoot.hovered) {
                return MenuButton.State.Hover
            }
            return MenuButton.State.Rest
        }

        PlasmaCore.FrameSvgItem {
            id: rest
            anchors.fill: parent
            visible: background.state == MenuButton.State.Rest
        ...
        PlasmaCore.FrameSvgItem {
            id: hover
            anchors.fill: parent
            visible: background.state == MenuButton.State.Hover
            imagePath: "widgets/menubaritem"
            prefix: "hover"
        }
        PlasmaCore.FrameSvgItem {
            id: down
            anchors.fill: parent
            visible: background.state == MenuButton.State.Down
            imagePath: "widgets/menubaritem"
            prefix: "pressed"
        }
    }
