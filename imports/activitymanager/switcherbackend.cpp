...
{
    registerShortcut(QString::fromLatin1("next activity"),
                     i18n("Walk through activities"),
                     Qt::META | Qt::Key_Tab,
                     &SwitcherBackend::keybdSwitchToNextActivity);

    registerShortcut(QString::fromLatin1("previous activity"),
...
void SwitcherBackend::trySetState(State newState, bool active)
{
    ...
    // if it's setting the current state
    if (!active) {
        if (old == newState) {
...
{
    if (value && m_shouldShowSwitcher & (Unneeded | KeyboardSwitch)) {
        m_shouldShowSwitcher = DropModeActive;
    }
