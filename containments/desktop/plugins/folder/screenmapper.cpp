...
int ScreenMapper::firstAvailableScreen(const QUrl &screenUrl, const QString &activity) const
{
    ...
    decltype(screens) matchedScreens;

    for (const std::pair<int, QString> &screen : std::as_const(screens)) {
        if (screen.second == activity) {
            matchedScreens.append(screen);
        }
    }

    auto comp = [](const std::pair<int, QString> &a, const std::pair<int, QString> &b) {
        return a.first < b.first;
    };
    const auto *newFirstScreen = std::min_element(screens.constBegin(), screens.constEnd(), comp);
    return newFirstScreen == screens.constEnd() ? -1 : newFirstScreen->first;
}
