...
MouseArea {
    ...
    Drag.onDragFinished: dropEvent => {
        if (dropEvent == Qt.MoveAction) {
            ...
            appletsModel.remove(currentApplet.index)
    ...
    onPositionChanged: mouse => {
        ...
        if (pressed) {
            ...
            const item = root.layoutManager.childAtCoordinat
            if (item && item.applet !== placeHolder) {es(mouse.x, mouse.y);
        ...
        mainItem: MouseArea {
            ...
            ColumnLayout {
                ...
                PlasmaComponents3.ToolButton {
                    ...
                    text: i18n("Move to a different panel...")
