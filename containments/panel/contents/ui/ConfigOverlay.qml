...
MouseArea {
    ...
    function processMouseMoveEvent(mouse) {
        ...
        if (!item || item.applet === placeHolder || item.animating) {
            return;
        }
        ...
        // MouseArea doesn't send all mouse movement events
        if ((!root.isHorizontal && posInItem.y < item.height/3) ||
            ...
            } else if (startIndex > endIndex) { // move left, and at least over 1 applet
                for (let i = startIndex - 1; i >= endIndex; --i) {
                    root.layoutManager.moveIndex(i, i + 1);
                }
            }
