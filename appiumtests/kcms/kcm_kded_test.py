...
def loadModules(session_bus: Gio.DBusConnection) -> list[str]:
...
class KCMTest(unittest.TestCase):
    ...
    def test_0_start_stop_service(self) -> None:
        ...
        self.driver.find_element(AppiumBy.NAME, "Accent Color")  # From plasma-workspace
        ...
        self.assertIn("plasma_accentcolor_service", loadModules(session_bus))  # The service id is from plasma-workspace

        stop_button.click()
        wait.until(EC.presence_of_element_located((AppiumBy.NAME, "Start Accent Color")))
        self.assertNotIn("plasma_accentcolor_service", loadModules(session_bus))
