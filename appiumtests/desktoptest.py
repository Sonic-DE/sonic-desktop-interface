...
class DesktopTest(unittest.TestCase):
    ...
    def test_4_bug477185_meta_number_shortcut(self) -> None:
        ...
        desktopfile_path: str = os.path.join(GLib.get_user_data_dir(), "applications", "org.kde.testwindow.desktop")
        with open(desktopfile_path, "w", encoding="utf-8") as file_handler:
            file_handler.write(f"""[Desktop Entry]
Exec=python3 {os.path.join(os.getcwd(), "resources", "org.kde.testwindow.py")}
...
""")
        ...
        message.set_body(GLib.Variant("(s)", [f"panels().forEach(containment => containment.widgets('org.kde.plasma.taskmanager').forEach(widget => widget.currentConfigGroup = ['General'];widget.writeConfig('launchers', 'file://{desktopfile_path}');))"]))
