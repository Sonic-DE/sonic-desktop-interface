...
class KickoffTests(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        lib_paths = lib_paths + ['/usr/lib64', '/usr/lib']
        for path in lib_paths:
            file = "{}/libexec/kactivitymanagerd".format(path)
            if os.path.exists(file):
                print("starting {}".format(file))
                self.kactivitymanagerd = subprocess.Popen([file])
                break
    ...
    def test_search_and_open(self):
        ...
        WebDriverWait(self.driver, 10).until(lambda x: self.driver.query_app_state('org.kde.plasma.emojier.desktop') == 4)
        self.assertEqual(self.driver.query_app_state('org.kde.plasma.emojier.desktop'), 4)
