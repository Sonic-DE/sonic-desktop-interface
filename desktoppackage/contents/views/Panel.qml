...
Item {
    ...
    Connections {
        ...
        function onActivated() {
            // BUG 472947: status changes to PassiveStatus or ActiveStatus after applet shortcut is pressed for the second time
            if (containment.status === PlasmaCore.Types.PassiveStatus /*After pressing panel shortcut*/ || containment.status === PlasmaCore.Types.ActiveStatus) {
                ...
                // BUG 472947: if applet shortcut is pressed, panel also gets activated, but status will change to RequiresAttentionStatus after applet has focus
