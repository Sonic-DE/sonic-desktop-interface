    ...
    , m_workingTouchpadFound(false)
{
    ...
    connect(&m_dependencies, &QDBusServiceWatcher::serviceRegistered, &TouchpadDisabler::serviceRegistered);

    connect(m_backend, &TouchpadBackend::touchpadStateChanged, &TouchpadDisabler::updateCurrentState);
    connect(m_backend, &TouchpadBackend::touchpadReset, &TouchpadDisabler::handleReset);
