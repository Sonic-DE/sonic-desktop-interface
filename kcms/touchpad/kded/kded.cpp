...
void migrateConfig(TouchpadBackend *backend)
{
    ...
    isWayland = backend->getMode() == TouchpadInputBackendMode::WaylandLibinput;
#endif
#if BUILD_KCM_TOUCHPAD_X11
    isX11 = backend->getMode() == TouchpadInputBackendMode::XLibinput;
#endif
    ...
    KConfigGroup migrationGroup = oldConfig->group(u"migration"_s);
    if (isX11 && migrationGroup.hasKey("MigratedToPlasma6_X11")) {
        return;
    }
    if (isWayland && migrationGroup.hasKey("MigratedToPlasma6_Wayland")) {
        return;
    }
    ...
    for (LibinputCommon *device : backend->inputDevices()) { // only touchpads, because TouchpadBackend
        ...
        if (device->supportsDisableWhileTyping()) {
            device->setDisableWhileTyping(disableOnKeyboardActivity);
        }
    }
    ...
    if (isX11) {
        migrationGroup.writeEntry("MigratedToPlasma6_X11", "touchpadxlibinputrc");
    }
    if (isWayland) {
        migrationGroup.writeEntry("MigratedToPlasma6_Wayland", "kcminputrc");
    }
    oldConfig->sync();
}
