...
bool Tablet::isDefaults() const
{
    ...
    auto cfg = KSharedConfig::openConfig("kcminputrc");
    auto group = cfg->group("ButtonRebinds").group("Tablet");
    if (group.isValid()) {
        return false;
    }
...
void Tablet::save()
{
    ...
    for (auto it = m_unsavedMappings.cbegin(), itEnd = m_unsavedMappings.cend(); it != itEnd; ++it) {
        auto group = cfg->group("ButtonRebinds").group("Tablet").group(it.key());
        for (auto itDevice = it->cbegin(), itDeviceEnd = it->cend(); itDevice != itDeviceEnd; ++itDevice) {
            group.writeEntry(QString::number(itDevice.key()), QStringList{"Key", itDevice->toString(QKeySequence::PortableText)}, KConfig::Notify);
        }
    }
}
...
void Tablet::defaults()
{
    ...
    KSharedConfig::openConfig("kcminputrc")->group("ButtonRebinds").deleteGroup("Tablet");
    m_unsavedMappings.clear();
    Q_EMIT buttonMappingChanged();
}
...
QKeySequence Tablet::padButtonMapping(const QString &deviceName, uint button) const
{
    ...
    auto cfg = KSharedConfig::openConfig("kcminputrc");
    auto group = cfg->group("ButtonRebinds").group("Tablet").group(deviceName);
