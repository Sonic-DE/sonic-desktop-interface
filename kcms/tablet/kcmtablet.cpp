    ...
    for (const auto &device : QStringList{"TabletRing"}) {
        for (auto it = m_unsavedMappings[device].cbegin(), itEnd = m_unsavedMappings[device].cend(); it != itEnd; ++it) {
            ...
            for (auto itDevice = it->cbegin(), itDeviceEnd = it->cend(); itDevice != itDeviceEnd; ++itDevice) {
                ...
                auto modeGroup = configGroup.group(QString::number(group));
...
{
    m_unsavedMappings["TabletRing"][deviceName][{group, group}] = keySequence;
    Q_EMIT settingsRestored();
}
...
InputSequence Tablet::padRingMapping(const QString &deviceName, uint button, uint group) const
{
    if (deviceName.isEmpty()) {
        return {};
    }

    if (const auto &device = m_unsavedMappings["TabletRing"][deviceName]; device.contains({button, group})) {
        return device.value({button, group});
    }

    const auto cfg = KSharedConfig::openConfig("kcminputrc");
    const auto configGroup = cfg->group("ButtonRebinds").group("TabletRing").group(deviceName).group(QString::number(group));
