...
SimpleKCM {
    ...
    Kirigami.FormLayout {
        ...
        Item {
            ...
            function resetOutputArea(mode, outputArea) {
                if (mode === 0) {
                    ...
                    tabletItem.x       = Qt.binding(function() {return outputArea.x      * outputItem.width })
                    tabletItem.y       = Qt.binding(function() {return outputArea.y      * outputItem.height })
                    ...
                    tabletSizeHandle.x = Qt.binding(function() {return outputArea.width  * outputItem.width })
                    tabletSizeHandle.y = Qt.binding(function() {return outputArea.height * outputItem.height })
                }
            }
        ...
        QQC2.CheckBox {
            id: keepAspectRatio
            text: i18nd("kcmtablet", "Lock aspect ratio")
            visible: outputAreaCombo.currentIndex >= 2
            checked: true
        }
        QQC2.Label {
            text: i18nd("kcmtablet", "%1,%2 - %3x%4", Math.floor(outputAreaView.outputAreaSetting.x * outputItem.outputSize.width)
