        ...
        delegate: ColumnLayout {
            ...
            Repeater {
                ...
                delegate: ActionBinding {
                    ...
                    name: i18ndc("kcm_tablet", "@info Meant to be inserted into an existing sentence like 'configuring pad ring 0'", "pad ring %1", ringDelegate.index + 1)
                    supportsPenButton: false
                    supportsRelativeEvents: true

                    function refreshInputSequence(): void {
                        seq.inputSequence = kcm.padRingMapping(root.padDevice.name, seq.index, ringDelegate.index)
                    }

                    inputSequence: kcm.padRingMapping(root.padDevice.name, seq.index, ringDelegate.index)
                    Connections {
                        target: kcm
                    ...
                    onGotInputSequence: sequence => {
                        kcm.assignPadRingMapping(root.padDevice.name, ringDelegate.index, seq.index, sequence)
                    }
