...
Item {
    ...
    function setOutputAreaMode(mode): void {
        if (mode === 0) {
            ...
            if (mode === 2) {
                console.info("output aspect ratio: " + outputAreaItem.aspectRatio);
                console.info("input aspect ratio: " + outputAreaView.device.size + " " + outputAreaView.device.height + " " + (outputAreaView.device.size.width / outputAreaView.device.size.height));
