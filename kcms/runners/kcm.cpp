    ...
    , m_config(KSharedConfig::openConfig("krunnerrc"))
{
    ...
    m_krunnerSettingsButton = new QPushButton(QIcon::fromTheme(QStringLiteral("krunner")), QStringLiteral("KRunner Settings"), this);
    connect(m_krunnerSettingsButton, &QPushButton::clicked, this, [this] {
        if (!m_krunnerSettingsDialog) {
            const KPluginMetaData krunnerModule(QPluginLoader("plasma/kcms/systemsettings/kcm_krunnersettings"));
