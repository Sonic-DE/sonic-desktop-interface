    ...
    connect(m_model, &QAbstractItemModel::dataChanged, this, [this] {
        ...
        for (int i = 0, count = m_model->rowCount(); i < count; ++i) {
            if (m_model->item(i)->data(Roles::PendingDeletionRole).toBool()) {
                m_pendingDeletions << m_model->item(i)->data(Roles::PluginNameRole).toString();
            }
        }
        setNeedsSave(m_data->settings()->isSaveNeeded() || !m_pendingDeletions.isEmpty());
        setRepresentsDefaults(m_data->settings()->isDefaults() && m_pendingDeletions.isEmpty());
    });
}
...
void KCMSplashScreen::processPendingDeletions()
{
    ...
    const QStringList pendingDeletions = m_pendingDeletions;
    for (const QString &plugin : pendingDeletions) {
        if (plugin == m_data->settings()->theme()) {
            Q_EMIT error(i18n("You can not delete the currently selected splash screen"));
            m_model->setData(m_model->index(pluginIndex(plugin), 0), false, Roles::PendingDeletionRole);
            m_pendingDeletions.removeOne(plugin);
            continue;
        }
