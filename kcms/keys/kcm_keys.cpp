...
{
    QModelIndex conflictingIndex;
    for (int i = 0; i < m_shortcutsModel->rowCount() && !conflictingIndex.isValid(); ++i) {
        const QModelIndex componentIndex = m_shortcutsModel->index(i, 0);
        for (int j = 0; j < m_shortcutsModel->rowCount(componentIndex) && !conflictingIndex.isValid(); ++j) {
            ...
            if (m_shortcutsModel->data(actionIndex, BaseModel::ActiveShortcutsRole).value<QSet<QKeySequence>>().contains(keySequence)) {
                conflictingIndex = actionIndex;
            }
        }
    }
    return  m_shortcutsModel->mapToSource(conflictingIndex);
}
