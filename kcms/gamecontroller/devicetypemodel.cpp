...
DeviceTypeModel::DeviceTypeModel()
{
    addType(SDL_CONTROLLER_TYPE_XBOXONE,
            ...
            DeviceType::Xbox,
            QStringLiteral("qrc://kcm/kcm_gamecontroller/artwork/xbox-optimized.svg"));
...
QString DeviceTypeModel::svgPath(int row)
{
    const int type = m_typeOrder.value(row);
    QString path;
    if (m_types.contains(type)) {
    ...
    qCDebug(KCM_GAMECONTROLLER) << "svgPath for row: " << row << " is: " << path;
