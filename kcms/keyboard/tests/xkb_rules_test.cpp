...
private Q_SLOTS:
    ...
    void testModel()
    {
        for (const ModelInfo *modelInfo : rules->modelInfos) {
    ...
    {
        for (const OptionGroupInfo *optionGroupInfo : rules->optionGroupInfos) {
    ...
    void testWriteNewXml()
    {
        ...
        for (const OptionGroupInfo *optionGroupInfo : rules->optionGroupInfos) {
            ...
            for (const OptionInfo *optionGroupInfo : optionGroupInfo->optionInfos) {
    ...
    void testRulesVersion()
    {
        ...
        for (const LayoutInfo *layoutInfo : rules11->layoutInfos) {
            QVERIFY(layoutInfo != nullptr);
            QVERIFY(!layoutInfo->name.isEmpty());
            QVERIFY(!layoutInfo->description.isEmpty());
