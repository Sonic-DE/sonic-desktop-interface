    ...
    , d(new K4TimeZoneWidget::Private)
{
    ...
    for (const auto &id : timezones) {
        ...
        // since there is no standard APi for city/continent names, we use the translation catalog of the digitalclock applet which contain these
        ...
        const QString city = i18nd(domain, id.mid(separator + 1));
        const QString key = city + '|' + continent + '|' + id;
        cities.append(key);
        zonesByCity.insert(key, QTimeZone(id));
    }
    std::sort(cities.begin(), cities.end(), localeLessThan);

    for (const QString &key : cities) {
