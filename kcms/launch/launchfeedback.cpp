...
LaunchFeedback::LaunchFeedback(QObject *parent, const QVariantList &args)
    ...
    , m_busyCursorTimeout(STARTUP_DEFAULT_TIMEOUT)
    , m_taskManagerNotification(true)
    , m_taskManagerNotificationTimeout(STARTUP_DEFAULT_TIMEOUT)
{
...
int LaunchFeedback::busyCursorTimeout() const
{
    return m_busyCursorTimeout;
}

void LaunchFeedback::setBusyCursorTimeout(int duration)
{
    if (m_busyCursorTimeout != duration) {
        m_busyCursorTimeout = duration;

        emit busyCursorTimeoutChanged();
...
int LaunchFeedback::taskManagerNotificationTimeout() const
{
    return m_taskManagerNotificationTimeout;
}

void LaunchFeedback::setTaskManagerNotificationTimeout(int duration)
{
    if (m_taskManagerNotificationTimeout != duration) {
        m_taskManagerNotificationTimeout = duration;

        emit taskManagerNotificationTimeoutChanged();
...
void LaunchFeedback::load()
{
    ...
    setBusyCursorTimeout(c.readEntry("Timeout", STARTUP_DEFAULT_TIMEOUT));
    ...
    setTaskManagerNotificationTimeout(c.readEntry("Timeout", STARTUP_DEFAULT_TIMEOUT));
}
...
void LaunchFeedback::defaults()
{
    ...
    setBusyCursorTimeout(STARTUP_DEFAULT_TIMEOUT);

    setTaskManagerNotification(true);
    setTaskManagerNotificationTimeout(STARTUP_DEFAULT_TIMEOUT);
}
...
void LaunchFeedback::updateNeedsSave()
{
    ...
    if (m_busyCursorTimeout != c.readEntry("Timeout", STARTUP_DEFAULT_TIMEOUT)) {
        needsSave = true;
    }
    ...
    if (m_taskManagerNotificationTimeout != c.readEntry("Timeout", STARTUP_DEFAULT_TIMEOUT)) {
        needsSave = true;
    }
