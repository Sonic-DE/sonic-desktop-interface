...
KLookAndFeel::Flavor LookAndFeelCrawler::variant() const
{
    return m_variant;
}

void LookAndFeelCrawler::setVariant(KLookAndFeel::Flavor variant)
{
...
void LookAndFeelCrawler::reload()
{
    ...
    for (const KPluginMetaData &metadata : packagesMetaData) {
        ...
        const auto flavor = KLookAndFeel::colorSchemeFlavor(pkg);
        if (flavor != KLookAndFeel::Flavor::Unknown && flavor != m_variant) {
            continue;
        }
