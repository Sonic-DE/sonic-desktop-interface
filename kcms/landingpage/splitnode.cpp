...
{
    if (m_light) {
        m_material->texture1 = m_light->texture();
        if (QSGDynamicTexture *dynamicTexture = qobject_cast<QSGDynamicTexture *>(m_light->texture())) {
            dynamicTexture->updateTexture();
        }
    }

    if (m_dark) {
        m_material->texture2 = m_dark->texture();
        if (QSGDynamicTexture *dynamicTexture = qobject_cast<QSGDynamicTexture *>(m_dark->texture())) {
            dynamicTexture->updateTexture();
        }
    }
...
void SplitNode::setDark(QSGTextureProvider *provider)
{
    if (m_dark == provider) {
        return;
    }
    ...
    if (m_dark) {
        disconnect(m_dark, &QSGTextureProvider::textureChanged, this, &SplitNode::markMaterialDirty);
    }

    m_dark = provider;
    if (m_dark) {
        connect(m_dark, &QSGTextureProvider::textureChanged, this, &SplitNode::markMaterialDirty, Qt::DirectConnection);
    }
}

void SplitNode::setLight(QSGTextureProvider *provider)
{
    if (m_light == provider) {
        return;
    }
    ...
    if (m_light) {
        disconnect(m_light, &QSGTextureProvider::textureChanged, this, &SplitNode::markMaterialDirty);
    }

    m_light = provider;
    if (m_light) {
        connect(m_light, &QSGTextureProvider::textureChanged, this, &SplitNode::markMaterialDirty, Qt::DirectConnection);
    }
}
