...
Kirigami.ApplicationItem {
    ...
    Kirigami.ScrollablePage {
        ...
        Kirigami.FormLayout {
            Repeater {
                id: buttonMappings
                property var extraButtons: Array.from({length: 24}, (value,index) =>
                    ({button: Qt["ExtraButton" + (index + 1)], label: i18ndc("kcmmouse", "@label for assigning an action to a numbered button", "Button %1:", index + 1)})
                )
                model: extraButtons.filter(button => device.supportedButtons & button.button)

                function load() {
                    for (var i = 0; i < count; ++i) {
                        itemAt(i).keySequence = device.getKeys(model[i].button)
                    }
                }
                ...
                delegate: KeySequenceItem {
                    ...
                    onKeySequenceChanged: {
                        if (!root.loading) {
                            device.setKeys(modelData.button, keySequence)
                            root.changeSignal()
                        }
                    }
                }
