...
Kirigami.ApplicationItem {
    ...
    pageStack.initialPage: Kirigami.ScrollablePage {
        ...
        Kirigami.FormLayout {
            ...
            QQC2.Button  {
                text: i18ndc("kcmmouse", "@action:button", "Rebind Additional Mouse Buttons…")
                visible: buttonMappings.model.length > 0
    ...
    Kirigami.ScrollablePage {
        ...
        Kirigami.FormLayout {
            Repeater {
                ...
                model: {
                    // Show configuration elements for all buttons that are supported by the current devices and additionally
                    // for those buttons that were configured in the past
                    const extraButtons = Array.from({length: 24}, (value,index) => ({
                        buttonName: "ExtraButton" + (index + 1),
                        label: i18ndc("kcmmouse", "@label for assigning an action to a numbered button", "Button %1:", index + 1)
                    }))
                    const configuredButtons = Object.keys(backend.buttonMapping).reduce((buttons, button) => buttons | button, 0)
                    const supportedButtons = Array.prototype.reduce.call(deviceModel, (buttons, device) => buttons | device.supportedButtons, 0)
                    return extraButtons.filter(button => Qt[button.buttonName] & (supportedButtons | configuredButtons))
                }

                function load() {
                    for (let i = 0; i < count; ++i) {
                        const keys = backend.buttonMapping[model[i].buttonName]
                        if (keys) {
                            itemAt(i).keySequence = keys
                        }
                    }
                }
