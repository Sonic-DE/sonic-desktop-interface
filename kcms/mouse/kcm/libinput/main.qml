...
Kirigami.ApplicationItem {
    ...
    property int deviceIndex: (
        (deviceSelector.currentIndex >= 0 && deviceSelector.currentIndex < backend.mouseDevices?.length)
            ? deviceSelector.currentIndex
            : -1
    )
    property QtObject device: deviceIndex >= 0 ? backend.mouseDevices[deviceIndex] : null
    signal changeSignal()
    ...
    pageStack.initialPage: Kirigami.ScrollablePage {
        ...
        Kirigami.FormLayout {
            id: formLayout
            enabled: backend.mouseDevices?.length > 0

            // Device
            QQC2.ComboBox {
                id: deviceSelector
                Kirigami.FormData.label: i18nd("kcmmouse", "Device:")
                visible: !backend.isAnonymousDevice
                enabled: count > 1
                Layout.fillWidth: true
                model: backend.mouseDevices?.length > 0 ? backend.mouseDevices : [""]
                textRole: "name"
            ...
            QQC2.Button  {
                ...
                visible: !backend.isAnonymousDevice && (
                    buttonMappings.model.length > 0 || Array.prototype.some.call(backend.mouseDevices, supportsExtraButtons)
                )
